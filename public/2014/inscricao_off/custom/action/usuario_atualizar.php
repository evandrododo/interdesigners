<?php
    ...
    if (BLACKLIST == 0 && !$blacklist_dao->isBlacklisted($usuario->get('email'))) {
        $return[] = array(
            "Action" => "Error",
            "Error" => "Você não está autorizado a utilizar este e-mail nesse sistema."
        );
    } else if (BLACKLIST == 1 && $blacklist_dao->isBlacklisted($usuario->get('email'))) {
        $return[] = array(
            "Action" => "Error",
            "Error" => "Você não está autorizado a utilizar este e-mail nesse sistema."
        );
