...
<?
		...
		?>
			<td class="<?=$venda['id']?>">
				<?=$venda['nome']?><br/>
				<input type='hidden' value="<?=$venda['nome']?>" name='snome'/>
				<?=$venda['email']?><br/>
				<input type='hidden' value="<?=$venda['email']?>" name='semail'/>
				<?=$venda['telefone']?><br/>
			</td>
			<td class='spacotes'>
				<?
			...
			<td>
				<input name='<?=$venda['vendaid']?>' type='checkbox' <? if($venda['perguntas']=='pago') echo 'checked="checked";'?> />
			</td>
		</tr>
		<?
	}	
